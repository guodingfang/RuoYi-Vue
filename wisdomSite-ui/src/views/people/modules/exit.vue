        ...
        <div class="r-warp">
          <div class="query-box">
            ...
            <div class="r-query">
              ...
              <el-select filterable clearable @change='getData()' v-model="workType" placeholder="全部工种">
                <el-option
              ...
              <el-select filterable clearable @change='getData()' v-model="personType" placeholder="全部职务/权限">
              ...
              <el-button type="primary" @click="handleExport(1)">
                导出已删除人员
              </el-button>
            </div>
          </div>
          <div class="table-warp">
            <div class="table-button">
              <el-button @click="personStatus()">批量恢复进场</el-button>
              <el-button @click="deleteStaff()">批量删除</el-button>
            </div>
            <el-table
              default-expand-all
              row-key="id"
            ...
            >
              ...
              >
                ...
                <template slot-scope="scope">
                  <el-button size="mini" type="text" @click="deleteStaff(scope.row.id)">删除</el-button>
                  <el-button size="mini" type="text"  @click="personStatus(scope.row.id)">恢复进场</el-button>
                </template>
              </el-table-column>
            </el-table>
            <Pagination
...
<script>
import { getTree, detailProjectUnit, deleteProjectUnit,
getTeam, setAllData,deleteStaff, } from "@/api/people/company";
import { getEtable, exportJob } from "@/api/people/exit";
...
export default {
  ...
  methods: {
    async initTree() {
      ...
      this.changeTree();
      this.paramInit = Object.assign({ ...this.queryParams, projectId: this.treeList[0].nodeId })
      this.getData(this.paramInit);
    },
    clickTree(data) {
      this.queryParams.keyWordName = null
      this.treeId = data.id;
      this.projectId = data.nodeId;
    ...
    async deleteStaff(id) {
      if (id) {
        ...
        if (this.tableSelection && this.tableSelection.length > 0) {
          let ids = (this.tableSelection.map(item => item.id)).join(',')
          return new Promise(res => {
            this.$confirm("是否确认删除?", "提示", {
              confirmButtonText: "确定",
              cancelButtonText: "取消",
              ...
              .then(async () => {
                const re = await deleteStaff(ids);
                if (re.code === 200) {
                  this.$message({
                    message: re.msg,
                    type: "success",
    ...
    changPage(val) {
      ...
      this.paramInit.pageSize = val.limit
      console.log(this.paramInit,val);
      this.getData()
    },
    async getData() {
      let param = {
      ...
      const re = await getEtable(param);
