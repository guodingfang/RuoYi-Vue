  ...
  <div :class="['camera-wrap', cusClass]">
    ...
    <div class="control-box">
      ...
      <div class="zoom-box mt20">
        <button class="control-btn zoom" @click="doControl('11')">缩小</button>
        <button class="control-btn zoom" @click="doControl('10')">放大</button>
      </div>
      <div class="zoom-box mt20">
...
<script>
  ...
  import { camControl, capture, getOnlineVideo, getVideo } from  '@/api/system/device';
  ...
  export default {
    ...
    methods: {
      getPlayUrl() {
        ...
        console.log('mode', this.videoEquipmentId)
        // getOnlineVideo({ equipmentSerialnumber, channelNumber, playMode: this.playMode }).then(res => {
        getVideo({ videoEquipmentId: this.videoEquipmentId }).then(res => {
          console.log(res.data.videoEquipmentAddress);
          // this.playUrl = res.msg
          this.liveInfo = {
            equipmentSerialnumber: res.data.equipmentSerialnumber,
            channelNumber: res.data.channelNumber,
          }
          this.playUrl = res.data.videoEquipmentAddress
          this.initPlayer()
        })
      },
      initPlayer() {
