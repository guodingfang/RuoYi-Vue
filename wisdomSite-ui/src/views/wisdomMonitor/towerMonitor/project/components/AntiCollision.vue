  ...
  <div class="ele-pos-wrap">
    <div class="ele-btn-wrap">
      <div class="btn-l">
        ...
        <el-button size="small" type="primary" @click="addTower">添加设备</el-button>
      </div>
      <div class="btn-r">
        ...
        <tower
          ...
          <div class="item">
            <el-popover
              ...
              <i  slot="reference" class="el-icon-delete del-icon" @click="delEle(element.id)"></i>
            </el-popover>
            {{element.name}}
          </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async getTowPosList() {
        const { rows } = await towerEquipmentList({
          pageNum: 1,
          pageSize: 99,
          presence: 1,
        });
        ...
        rows.map(e => {
          if(!!e.radius){
            elements.push({
              id: e.id, x: Number(e.coordinateX), y: Number(e.coordinateY), r: Number(e.radius), name: e.equipmentName
            })
          }
        })
      ...
      getSel(e) {
        this.elements.push({
          id: e.id, x: 250, y: 250, r:100,  name: e.equipmentName||'暂时没名字',
        });
      },
