<template>
  <div>
      ...
      <template v-slot:operate>
        <el-table-column label="操作" align="left">
          <template slot-scope="scope">
            ...
            <span  class="jl-green mr10 pointer" @click="openApplyDialog(scope.row)">再次申请</span>
          </template>
        </el-table-column>
      </template>
    </Table>
    ...
    />
    <re-apply-dialog
      :dialogFlag.sync="applyFlag"
      :id="operateId"
      :acceptForms="applyData"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /** 打开新增框*/
      openApplyDialog(rows) {
        this.operateId = Number(rows.id);
        this.applyData = rows
        this.applyFlag = true;
      },
      /** 打开详情框*/
