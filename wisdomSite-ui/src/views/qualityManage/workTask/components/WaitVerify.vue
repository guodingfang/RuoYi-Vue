  ...
  <div>
    ...
    >
      ...
      <template v-slot:qualityInfo>
        <el-table-column label="质量问题等级 | 问题类型 | 问题描述" width="240px">
          <template slot-scope="scope">
      ...
      <template v-slot:operate>
        <el-table-column label="操作" align="left">
          <template slot-scope="scope">
            ...
            <span  class="jl-green mr10 pointer" @click="openVerifyDialog(scope.row.id)">进行验证</span>
          </template>
        </el-table-column>
      </template>
    </Table>
...
<script>
  ...
  export default {
    ...
    data() {
      return{
        tableConfig: [
          ...
          { prop: 'repairDate', label: '整改时间', width: ''},
        ],
        total: 0,
        tableData: [],
        //项目查询参数
    ...
    methods: {
      /** 查询 列表 */
      getQualityList() {
        getVerifyList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
