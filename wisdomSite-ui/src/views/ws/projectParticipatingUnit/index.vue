  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['ws:projectParticipatingUnit:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['ws:projectParticipatingUnit:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ws:projectParticipatingUnit:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ws:projectParticipatingUnit:export']"
        >导出</el-button>
      </el-col>
    </el-row>

    <el-table v-loading="loading" :data="projectParticipatingUnitList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否删除" align="center" prop="idDelete" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否删除" prop="idDelete">
          <el-input v-model="form.idDelete" placeholder="请输入是否删除" />
        </el-form-item>
      </el-form>
...
<script>
import { listProjectParticipatingUnit, getProjectParticipatingUnit, delProjectParticipatingUnit, addProjectParticipatingUnit, updateProjectParticipatingUnit, exportProjectParticipatingUnit } from "@/api/ws/projectParticipatingUnit";

export default {
  name: "ProjectParticipatingUnit",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProjectParticipatingUnit(this.queryParams).then(response => {
        this.projectParticipatingUnitList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getProjectParticipatingUnit(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateProjectParticipatingUnit(this.form).then(response => {
            ...
            addProjectParticipatingUnit(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除项目参建单位 编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delProjectParticipatingUnit(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有项目参建单位 数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportProjectParticipatingUnit(queryParams);
        }).then(response => {
