<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="是否删除" prop="projectId">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ws:teamPerson:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ws:teamPerson:export']"
        >导出</el-button>
      </el-col>
    </el-row>

    <el-table v-loading="loading" :data="teamPersonList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="ID" align="center" prop="id" />
      <el-table-column label="ID" align="center" prop="projectId" />
      <el-table-column label="参建单位ID" align="center" prop="unitId" />
      ...
      <el-table-column label="进场日期" align="center" prop="inDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.inDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="退场日期" align="center" prop="outDate" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column label="性别 男：1，女：0" align="center" prop="sex" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="是否删除" prop="projectId">
          <el-input v-model="form.projectId" placeholder="请输入是否删除" />
        </el-form-item>
        <el-form-item label="参建单位ID" prop="unitId">
          <el-input v-model="form.unitId" placeholder="请输入参建单位ID" />
        </el-form-item>
        <el-form-item label="班组ID" prop="teamId">
        ...
        <el-form-item label="退场日期" prop="outDate">
          <el-date-picker clearable size="small" style="width: 200px"
        ...
        <el-form-item label="性别 男：1，女：0">
          <el-select v-model="form.sex" placeholder="请选择性别 男：1，女：0">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
...
<script>
import { listTeamPerson, getTeamPerson, delTeamPerson, addTeamPerson, updateTeamPerson, exportTeamPerson } from "@/api/ws/teamPerson";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        unitId: [
          { required: true, message: "参建单位ID不能为空", trigger: "blur" }
        ],
        teamId: [
        ...
        sex: [
          { required: true, message: "性别 男：1，女：0不能为空", trigger: "blur" }
        ],
        nationCode: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTeamPerson(this.queryParams).then(response => {
        this.teamPersonList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTeamPerson(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateTeamPerson(this.form).then(response => {
            ...
            addTeamPerson(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除班组人员 编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delTeamPerson(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有班组人员 数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportTeamPerson(queryParams);
        }).then(response => {
